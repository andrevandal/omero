...
import { logger, fullUrl } from '@omero/utils'
...
export async function sendNotification<T extends NotificationType>(
  ...
  switch (channel) {
    case 'email': {
      ...
      const resend = new Resend(environment.RESEND_KEY)

      const { Component, getSubject, propsSchema } = template

      const emailProperties = v.parse(propsSchema, {
        ...properties,
        poweredByLink: fullUrl(environment.BASE_URL, '/').toString(),
        baseUrl: fullUrl(environment.BASE_URL, '/').toString(),
        language: 'en'
