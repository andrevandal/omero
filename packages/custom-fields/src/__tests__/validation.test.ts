...
describe('Custom Fields Validation', () => {
  ...
  test('select field with options', () => {
    ...
    expect(validateCustomFields({ status: 'draft' }, schema).success).toBe(true)
  })
  ...
  test('number field with min/max', () => {
    ...
    expect(validateCustomFields({ readingTime: 0 }, schema).success).toBe(false)
...
describe('Advanced Custom Field Types', () => {
  ...
  test('list field with item validation', () => {
    ...
    expect(
      validateCustomFields({ tags: Array.from({length: 11}).fill('tag') }, schema).success
